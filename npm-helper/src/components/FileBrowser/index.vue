  ...
  <div class="file-browser-root">
    ...
    >
      <div class="file-browser">
        <SearchList v-if="search" :files-list="allFullPaths" :need-search="search" @show-result="hasChar($event)"></SearchList>
        <!-- main part 这里的 v-if 真的恐怖！-->
        <ul v-show="showList" class="fb-wrap-viewer" @scroll="onScroll">
          <Fileslist :dir="pkgName" :list="filesData" @item-click="whenClickIt($event)" @dom-mounted="allFullPaths = $event" />
        </ul>
      </div>
    </div>
  </div>
...
export default class FileBrowser extends Vue {
  ...
  constructor() {
    ...
    this.directoryListing
      ? (this.filesData = {
          ...
          depth: -1,
          children: this.deepTrans(this.directoryListing, 0)
        })
  ...
  // helper FUNC
  private filesAdd(arr: Array<any>, depth: number) {
    const tempArr: any[] = []
  ...
  // 非字符即数组
  private deepTrans(arr: Array<any>, depth: number) {
    let tempArr: any[] = []
    // const Flist = FileBrowser.list;
    // let depth = pDep === 0 ? 0 : depth;

    if (typeof arr[0] === 'string' && typeof arr[1] !== 'object') {
      // const item = arr[i];
      tempArr = this.filesAdd(arr, pDep)
      return tempArr
    }
    ...
    if (typeof arr[0] !== 'string') {
      for (let i = 0; i < arr.length; i++) {
        if (typeof arr[i] !== 'string') {
          const data = this.deepTrans(arr[i], depth)
          pDep -= 1
          tempArr.push({
            name: arr[i][0],
            type: 'folder',
    ...
    if (typeof arr[0] === 'string' && typeof arr[1] !== 'string') {
      ...
      pDep += 1
      const data = this.deepTrans(arr[1], pDep)
      tempArr.push({
        name: arr[0],
        type: 'folder',
        depth: pDep + 1,
