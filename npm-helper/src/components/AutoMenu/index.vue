<template>
  <div v-if="showTOC" class="navi-toc">
    <ul v-show="showNavi" :style="wrapStyle">
      <div class="input-wrap">
        <input ref="searchInput" v-model="searchVal" type="text" placeholder="搜索标题" />
      </div>
      <div class="i-placeholder"></div>
      <li
      ...
      >
        <a :title="text" @click="gotoAnchor(anchor, index)">{{ text }}</a>
        <!-- :href="'#'+anchor" -->
      </li>
    </ul>
    <span :class="showNavi ? 'icons i-minus' : 'icons i-plus'" @click="collapse"></span>
  </div>
</template>
...
export default class AutoMenu extends Vue {
  ...
  @Prop({ type: Object, default: () => ({}) }) wrapStyle!: object
  ...
  mounted() {
    ...
    const HTagEls = container.querySelectorAll(levelTags) as NodeListOf<HTMLHeadElement>
    !container
      ? console.error('selector error!')
      : HTagEls.forEach((el, i) => {
          this.naviItems.push({
            tagName: el.tagName.toLowerCase(),
            anchor: el.id ? el.id : (el.setAttribute('id', String(i)), String(i)),
  ...
  gotoAnchor(anchor: string, index: number) {
    if (this.activeLiIndex !== index) {
      ...
      document.getElementById(anchor)!.scrollIntoView({ behavior: 'auto', block: 'center' })
      setTimeout(() => {
        this.fromAnchor = false
      }, 1000)
    }
  ...
  naviLi() {
    ...
    if (naviEl) {
      const offsetTop = (naviEl.childNodes[this.activeLiIndex + 1] as HTMLLIElement).offsetTop
