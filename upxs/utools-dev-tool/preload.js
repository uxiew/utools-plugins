...
window.api = (e,t)=>{
    ...
    if (e.indexOf('/releases/') >= 0) {
        return Promise.resolve({
            previews: ['https://5b0988e595225.cdn.sohucs.com/images/20171221/22b1117f31ed4805b2087af994feb376.jpeg']
        })
    }
...
window.services = {
    ...
    readDevPluginLogo: e=>{
        ...
        if ("object" != typeof n)
            throw new Error("plugin.json 文件内容错误");
        if (!n.logo)
        ...
        if (!fs.existsSync(i))
            throw new Error('配置的 logo "' + i + '" 不存在');
        return i
    }
    ,
    getFileBlob: e=>{
