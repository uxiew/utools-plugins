...
async function getRandomNumber() {
	const res = await fetch(`https://developer.mozilla.org/en-US/search-index.json`,{
	headers:{
...
function handleClick() {
	promise = downloadFileAsync(`https://developer.mozilla.org/en-US/search-index.json`,`${utools.getPath('userData')}/plugins/mdn-index.json`);
}
